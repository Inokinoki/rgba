...
fn main() {
    ...
    // Boot and navigate through intro
    for _ in 0..240 {
    ...
    // Walk around the farm - use directions to explore
    for step in 0..80 {
        ...
        if step % 10 == 0 {
            save_bmp(&framebuffer, &format!("/tmp/walk_{}.bmp", step));
            ...
            println!("Walk step {}: {} colors", step, unique.len());
        }
    }

    // Convert all to PNG for analysis
