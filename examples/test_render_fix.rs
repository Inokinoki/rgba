...
fn main() {
    ...
    // Initial screenshot
    for _ in 0..300 { gba.run_frame_parallel(&mut framebuffer); frame += 1; }
    save_bmp(&framebuffer, "/tmp/fix_initial.bmp");
    let mut colors = std::collections::HashMap::new();
    for &p in &framebuffer { *colors.entry(p).or_insert(0u32) += 1; }
    println!("Initial (300): {} unique colors", colors.len());
    ...
    for round in 0..200 {
        ...
        if round % 20 == 19 {
            colors.clear();
            for &p in &framebuffer { *colors.entry(p).or_insert(0u32) += 1; }
            println!("Round {}: {} unique colors", round, colors.len());
            if colors.len() > 5 {
                save_bmp(&framebuffer, &format!("/tmp/fix_r{}.bmp", round));
            }
        }
    }
    save_bmp(&framebuffer, "/tmp/fix_final.bmp");
    println!("Saved screenshots");
}
