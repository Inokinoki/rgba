...
fn cpu_initializes_with_known_register_values() {
    ...
    assert_eq!(
        ...
        true,
        "IRQ should be enabled on reset"
    );
...
fn arm_branch_instructions_change_program_flow() {
    ...
    // B instruction with 24-bit offset 0x14 (condition AL, bits 27-25=101)
    // Encoding: 0xEA000012 = B +80
    let mut rom = vec![0u8; 0x200];
    let insn = 0xEA_00_00_12u32.to_le_bytes();
    rom[0..4].copy_from_slice(&insn);
    mem.load_rom(rom);

    // When: Branch forward with offset 0x12 (18) * 4 = 72 bytes
    // Target = instruction_addr + 8 + 72 = 0x0800_0000 + 0x50 = 0x0800_0050
    cpu.step(&mut mem);
...
fn arm_memory_instructions_read_and_write_memory() {
    ...
    assert_eq!(
        cpu.get_reg(2),
        0xDEAD_BEEF,
        "R2 should contain loaded value"
    );
}
