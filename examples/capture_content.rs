...
fn main() {
    ...
    for round in 0..200 {
        ...
        // Save screenshot every frame from round 70-90
        if round >= 70 && round <= 90 {
            let mut unique = std::collections::HashMap::new();
            for &p in framebuffer.iter() { *unique.entry(p).or_insert(0u32) += 1; }
            if unique.len() > 3 {
                save_bmp(&framebuffer, &format!("/tmp/game_r{}.bmp", round));
                println!("Round {}: {} colors, saved", round, unique.len());
            }
        }
    }
}
