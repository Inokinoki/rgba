...
fn main() {
    ...
    for round in 0..200 {
        ...
        if round >= 70 && round <= 92 {
            ...
            let num_colors = unique_colors.len();
            let non_black = framebuffer.iter().filter(|&&p| p != 0).count();

            if num_colors > 3 {
                save_bmp(&framebuffer, &format!("/tmp/detail_r{}.bmp", round));
            }

            println!(
                "Round {}: DISPCNT={:#06X} colors={} non_black={}",
                round, dispcnt, num_colors, non_black
            );
        }
    }
}
