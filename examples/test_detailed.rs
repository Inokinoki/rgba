...
fn main() {
    let tests = [
        ...
        ("nes", "gba-tests/nes/nes.gba"),
        ("ppu-hello", "gba-tests/ppu/hello.gba"),
        ("ppu-shades", "gba-tests/ppu/shades.gba"),
        ("ppu-stripes", "gba-tests/ppu/stripes.gba"),
        ("save-none", "gba-tests/save/none.gba"),
        ("save-sram", "gba-tests/save/sram.gba"),
        ("save-flash64", "gba-tests/save/flash64.gba"),
        ("save-flash128", "gba-tests/save/flash128.gba"),
    ];
    ...
    for (name, path) in tests {
        ...
        if r12 == 0 {
            println!("✅ PASS");
            println!(
                "               Steps: {} | PC: 0x{:08X} | Mode: {}",
                steps_taken, final_pc, mode
            );
            total_passed += 1;
        } else {
            println!("❌ FAIL");
