...
fn main() {
    ...
    // Press A with longer waits (like a player would)
    for i in 0..200 {
        ...
        if i % 25 == 0 {
            ...
            println!("A-{}: {} colors", i, unique.len());
        }
    }

    save_bmp(&framebuffer, "/tmp/after_a200.bmp");
    ...
    for step in 0..60 {
        ...
        if step % 10 == 0 {
            save_bmp(&framebuffer, &format!("/tmp/play_{}.bmp", step));
            let unique: std::collections::HashMap<u32, u32> =
                framebuffer
                    .iter()
                    .fold(std::collections::HashMap::new(), |mut m, &p| {
